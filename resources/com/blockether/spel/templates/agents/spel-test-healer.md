...
Each ref includes `[pos:X,Y W×H]` — pixel coordinates (X,Y from top-left) and dimensions (width×height). Use for layout verification, overlap detection, viewport fit, and spatial reasoning.
