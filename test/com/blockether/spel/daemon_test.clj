...
;; =============================================================================
;; Unit Tests — build-structured-refs with selectors
;; =============================================================================

(defdescribe build-structured-refs-selectors-test
  "Unit tests for build-structured-refs including :id and :classes"

  (describe "includes id and classes when present"
    (it "passes through :id from refs"
      (let [refs {"e1" {:role "button" :name "Submit" :tag "button"
                        :bbox {:x 0 :y 0 :width 80 :height 30}
                        :id "submit-btn"}}
            result (#'sut/build-structured-refs refs)]
        (expect (= "submit-btn" (:id (get result "e1"))))))

    (it "passes through :classes from refs"
      (let [refs {"e1" {:role "button" :name "Submit" :tag "button"
                        :bbox {:x 0 :y 0 :width 80 :height 30}
                        :classes "btn btn-primary"}}
            result (#'sut/build-structured-refs refs)]
        (expect (= "btn btn-primary" (:classes (get result "e1"))))))

    (it "omits id and classes when not present"
      (let [refs {"e1" {:role "button" :name "Submit" :tag "button"
                        :bbox {:x 0 :y 0 :width 80 :height 30}}}
            result (#'sut/build-structured-refs refs)]
        (expect (nil? (:id (get result "e1"))))
        (expect (nil? (:classes (get result "e1"))))))))
