...
(defdescribe build-inject-js-test
  ...
  (describe "basic JS generation"
    (it "produces a non-empty JS string"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js test-refs {})]
                                     (expect (string? js))
                                     (expect (pos? (count js))))))
  ...
  (describe "compact label placement"
    (it "places label inside box for tall elements (dh >= 16)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "button" :name "OK" :bbox {:x 10 :y 20 :width 60 :height 25}}}
    ...
    (it "places label to the right for small elements (dh < 16)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "link" :name "X" :bbox {:x 10 :y 20 :width 40 :height 12}}}
                                         js   (build-inject-js refs {})]
  ...
  (describe "option toggles"
    (it "excludes boxes when :show-boxes false"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js test-refs {:show-boxes false})]
    ...
    (it "excludes labels when :show-badges false"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js test-refs {:show-badges false})]
        ;; No label elements
    ...
    (it "excludes dimensions from label when :show-dimensions false"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js test-refs {:show-dimensions false})]
    ...
    (it "generates minimal JS with all options disabled"
      ...
      (core/with-testing-page [pg] (let [js-all  (build-inject-js test-refs {})
  ...
  (describe "edge cases"
    (it "handles empty refs map"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js {} {})]
                                     (expect (string? js))
    ...
    (it "handles single ref"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js {"e1" {:role "button" :name "OK"
    ...
    (it "skips refs with zero-size bbox"
      ...
      (core/with-testing-page [pg] (let [js (build-inject-js {"e1" {:role "button" :name "Hidden"
...
(defdescribe filter-annotatable-test
  ...
  (describe "structural role filtering"
    (it "keeps interactive roles (button, link, textbox)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "button" :bbox {:x 0 :y 0 :width 50 :height 20}}
    ...
    (it "keeps content anchors (heading, img)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "heading" :bbox {:x 0 :y 0 :width 100 :height 30}}
    ...
    (it "removes pure structural roles (list, region)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "list"   :bbox {:x 0 :y 30 :width 300 :height 100}}
    ...
    (it "keeps listitem role (annotatable for list content)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "listitem" :bbox {:x 0 :y 30 :width 300 :height 25}}}
                                         result (sut/filter-annotatable refs)]
    ...
    (it "keeps text containers (paragraph, span)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "paragraph" :bbox {:x 0 :y 0 :width 300 :height 20}}
    ...
    (it "keeps text role (generic divs with text content)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "text" :bbox {:x 0 :y 0 :width 200 :height 20}}}
                                         result (sut/filter-annotatable refs)]
                                     (expect (= 1 (count result)))
                                     (expect (contains? result "e1")))))
  ...
  (describe "containment dedup"
    (it "removes container whose bbox fully wraps a child"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "navigation" :bbox {:x 0 :y 0 :width 500 :height 100}}
    ...
    (it "keeps both when neither contains the other"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "button" :bbox {:x 0  :y 0  :width 60 :height 20}}
    ...
    (it "handles identical bboxes by keeping both elements"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "button"  :bbox {:x 10 :y 10 :width 60 :height 20}}
    ...
    (it "keeps mixed-content container when it wraps a child (has own text)"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "paragraph" :mixed true :bbox {:x 0 :y 0 :width 300 :height 20}}
    ...
    (it "suppresses non-mixed container even if same role"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "text" :bbox {:x 0 :y 0 :width 300 :height 20}}
                                               "e2" {:role "text" :bbox {:x 5 :y 2 :width 60  :height 16}}}
                                         result (sut/filter-annotatable refs)]
    ...
    (it "zero-area child does not suppress visible container"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "link" :bbox {:x 63 :y 0 :width 0 :height 0}}
    ...
    (it "handles empty refs"
      ...
      (core/with-testing-page [pg] (expect (= {} (sut/filter-annotatable {})))))
  ...
  (describe "combined filtering (structural + containment)"
    (it "filters example.org-like structure: heading + paragraphs + link"
      ...
      (core/with-testing-page [pg] (let [refs {"e1" {:role "heading"   :bbox {:x 0 :y 50 :width 768 :height 45}}
...
(defdescribe visible-refs-test
  ...
  (describe "viewport filtering"
    (it "includes fully visible elements"
      ...
      (core/with-testing-page [pg] (let [vp   {:width 1024 :height 768}
    ...
    (it "includes partially visible elements (overlapping right edge)"
      ...
      (core/with-testing-page [pg] (let [vp   {:width 100 :height 100}
...
(defdescribe scope-helpers-test
  ...
  (describe "ref-scope? detection"
    (it "recognizes @e1 as a ref scope"
      ...
      (core/with-testing-page [pg] (expect (true? (#'sut/ref-scope? "@e1")))))

    (it "rejects bare ref without @ prefix"

      (core/with-testing-page [pg] (expect (false? (#'sut/ref-scope? "e1")))))

    (it "rejects e123 without @ prefix"

      (core/with-testing-page [pg] (expect (false? (#'sut/ref-scope? "e123")))))

    (it "rejects CSS selectors"

      (core/with-testing-page [pg] (expect (false? (#'sut/ref-scope? "#main")))
  ...
  (describe "resolve-scope"
    (it "converts @e1 to data-pw-ref selector"
      ...
      (core/with-testing-page [pg] (expect (= "[data-pw-ref=\"e1\"]" (#'sut/resolve-scope "@e1")))))

    (it "passes bare e1 through as CSS (no @ prefix)"

      (core/with-testing-page [pg] (expect (= "e1" (#'sut/resolve-scope "e1")))))

    (it "passes CSS selectors through unchanged"

      (core/with-testing-page [pg] (expect (= "#main" (#'sut/resolve-scope "#main")))
