...
(defdescribe snapshot-test
  ...
  (describe "basic snapshot"
    ...
    (it "parses snapshot -E selectors flag"
      (let [c (cmd ["snapshot" "-E"])]
        (expect (= "snapshot" (:action c)))
        (expect (true? (:selectors c)))))

    (it "parses snapshot --selectors flag"
      (let [c (cmd ["snapshot" "--selectors"])]
        (expect (= "snapshot" (:action c)))
        (expect (true? (:selectors c)))))

    (it "parses snapshot with selectors combined with other flags"
      (let [c (cmd ["snapshot" "-i" "-E" "-S"])]
        (expect (= "snapshot" (:action c)))
        (expect (true? (:interactive c)))
        (expect (true? (:selectors c)))
