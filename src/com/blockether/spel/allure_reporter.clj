        ...
        (when-not (str/includes? content "id=\"spel-md-renderer\"")
          (let [css
                ...
                (str
                  ...
                  ;; Handles: headings (# through ######), fenced code blocks,
                  ;; ordered / unordered lists, blockquotes, inline bold / italic
                  ;; / code / links, horizontal rules, and the HTTP exchange
                  ;; cards when the doc follows the `## METHOD URL → STATUS`
                  ;; convention.
                  "function inline(s){\n"
                  "  return esc(s)"
                  ...
                  "var sec=(lvl===3)?sectionType(hm[2].trim()):null;"
                  "if(sec){tokens.push({t:'section',sec:sec});continue}"
                  "tokens.push({t:'h'+lvl,text:hm[2]});continue}\n"
                  "    var ul=L.match(/^\\s*[-*]\\s+(.*)$/);\n"
                  "    if(ul){tokens.push({t:'li',kind:'ul',text:inline(ul[1])});continue}\n"
                  "    var ol=L.match(/^\\s*(\\d+)\\.\\s+(.*)$/);\n"
                  "    if(ol){tokens.push({t:'li',kind:'ol',text:inline(ol[2])});continue}\n"
                  "    var bq=L.match(/^>\\s?(.*)$/);\n"
                  "    if(bq){tokens.push({t:'bq',text:inline(bq[1])});continue}\n"
                  "    if(L.trim()===''){tokens.push({t:'blank'});continue}\n"
                  "    tokens.push({t:'p',text:inline(L)});\n"
                  "  }\n"
                  ...
                  "        else if(/^h[1-6]$/.test(ct.t))out.push('<'+ct.t+'>'+esc(ct.text)+'</'+ct.t+'>');\n"
                  "        j++;\n"
                  "      }\n"
                  "      out.push('</div>');\n"
                  "      continue;\n"
                  "    }else if(tk.t==='code'){closeList();closeCard();out.push(codeBlock(tk.lang,tk.text))}\n"
                  "    else if(tk.t==='hr'){closeList();closeCard();out.push('<hr>')}\n"
                  "    else if(/^h[1-6]$/.test(tk.t)){closeList();closeCard();out.push('<'+tk.t+'>'+esc(tk.text)+'</'+tk.t+'>')}\n"
                  "    else if(tk.t==='li'){\n"
                  "      if(curList!==tk.kind){closeList();curList=tk.kind;out.push('<'+curList+'>')}\n"
                  "      out.push('<li>'+tk.text+'</li>');\n"
                  "    }\n"
                  "    else if(tk.t==='bq'){closeList();out.push('<blockquote>'+tk.text+'</blockquote>')}\n"
                  "    else if(tk.t==='blank'){closeList()}\n"
                  "    else if(tk.t==='p'){closeList();out.push('<p>'+tk.text+'</p>')}\n"
                  "    j++;\n"
                  "  }\n"
                  "  closeList();closeCard();\n"
                  "  return out.join('\\n');\n"
...
(defn- ct-report-dir
  "Report output directory for clojure.test runs."
  ^String []
  (or (System/getProperty "allure.clojure-test.report-dir")
    (System/getenv "ALLURE_CLOJURE_TEST_REPORT_DIR")
    "allure-report"))

;; State

(def ^:private ct-run-state
  "Mutable state for the current test run. nil when not active."
  (atom nil))

(def ^:private ct-test-state
  "Mutable state for the current test var. nil between tests."
  (atom nil))

(def ^:private ct-counters
  (atom {:test 0 :pass 0 :fail 0 :error 0}))

(def ^:private ct-fixture-originals
  "Original :each fixtures per namespace, for restoration on :end-test-ns."
  (atom {}))

(def ^:private ct-pending-results
...
(defn- ct-build-labels
  [ts sub-suite]
  (let [ns-name  (:ns-name ts)
        pkg      (when ns-name (ns-package ns-name))
        hn       (:hostname @ct-run-state)]
    (cond-> []
      ns-name   (conj {:name "suite" :value ns-name})
      pkg       (conj {:name "parentSuite" :value pkg})
...
(defn- ct-write-test-result!
  [ts]
  (let [dir         (:output-dir @ct-run-state)
        result-uuid (uuid)
        ...
        video-att   (when-let [vp (:allure/video-path ts)]
                      (copy-file-attachment! dir vp "Video Recording"
                        "video/webm" ".webm"))
        io-atts     (filterv some? [out-att err-att trace-att har-att video-att])
        all-labels  (into (ct-build-labels ts sub-suite) ctx-labels)
        all-links   (or (seq ctx-links) [])
        all-params  (or (seq ctx-params) [])
        all-atts    (into (vec io-atts) ctx-atts)
        status-det  (ct-build-status-details ts)
        result      (cond-> {:uuid        result-uuid
                             :historyId   (md5-hex full-name)
                             :testCaseId  (md5-hex full-name)
                             :fullName    full-name
                             :name        (ct-build-display-name ts common-ctx)
                             :status      status
                             :stage       "finished"
                             :start       start-ms
                             :stop        stop-ms
                      ...
                      ctx-desc       (assoc :description ctx-desc))]
    (swap! ct-pending-results conj result)))

;; Lifecycle

(defn- ensure-ct-run-started!
  "Lazy-init on first :begin-test-ns. Creates output dir, resets state."
  []
  (when-not @ct-run-state
    (let [dir-path (ct-output-dir-path)
      ...
      (when (ct-clean?)
        (when (.exists dir)
          (doseq [^File f (reverse (file-seq dir))]
            (.delete f))))
      (.mkdirs dir)
      (reset! ct-run-state {:hostname        (hostname)
                            :start-ms        (System/currentTimeMillis)
                            :output-dir      dir
                            :output-dir-path dir-path})
      (reset! ct-counters {:test 0 :pass 0 :fail 0 :error 0})
      (reset! ct-fixture-originals {})
      (reset! ct-pending-results [])
      (allure/set-reporter-active! true))))

(defn- flush-ct-pending-results!
...
(defn- ct-on-begin-ns [m]
  (let [ns-obj   (:ns m)
    ...
    (swap! ct-run-state assoc :current-ns (str (ns-name ns-obj)))))

(defn- ct-on-end-ns [m]
  (let [ns-obj   (:ns m)
        original (get @ct-fixture-originals ns-obj)]
    ;; Restore original fixtures
    (if original
      (alter-meta! ns-obj assoc ::ct/each-fixtures original)
      (alter-meta! ns-obj dissoc ::ct/each-fixtures))
    (swap! ct-fixture-originals dissoc ns-obj)
    (swap! ct-run-state dissoc :current-ns)))

(defn- ct-on-begin-var [m]
  (let [v       (:var m)
        ns-name (str (ns-name (:ns (meta v))))
        tname   (str (:name (meta v)))]
    (swap! ct-counters update :test inc)
    (reset! ct-test-state
      {:var           v
       :ns-name       ns-name
       :test-name     tname
...
(defn- ct-on-pass [m]
  (swap! ct-counters update :pass inc)
  (when @ct-test-state
    (swap! ct-test-state update :assertions conj
      {:type     :pass
...
(defn- ct-on-fail [m]
  (swap! ct-counters update :fail inc)
  (when @ct-test-state
    (swap! ct-test-state
      (fn [s]
        (-> s
...
(defn- ct-on-error [m]
  (swap! ct-counters update :error inc)
  (when @ct-test-state
    (swap! ct-test-state
      (fn [s]
        (-> s
...
(defn- ct-on-end-var [_m]
  (when-let [ts @ct-test-state]
    (let [ts (cond-> ts
      ...
      (reset! ct-test-state (assoc ts :ended? true))
...
(defn- ct-on-summary [_m]
  (let [{:keys [test pass fail error]} @ct-counters]
    (println)
    (println (str "clojure.test Allure: "
               ...
               fail " failed, "
               error " errors"))
    (println (str "Results written to " (:output-dir-path @ct-run-state) "/")))
  (finalize-ct-run!))

;; Global defmethod hooks

(defonce ^:private ct-originals
...
(defmethod ct/report :begin-test-ns [m]
  (when (ct-enabled?)
    (ensure-ct-run-started!)
    (ct-on-begin-ns m))
  ((:begin-test-ns ct-originals) m))

(defmethod ct/report :end-test-ns [m]
  (when (ct-enabled?)
    (ct-on-end-ns m))
  ((:end-test-ns ct-originals) m))

(defmethod ct/report :begin-test-var [m]
  (when (ct-enabled?)
    (ct-on-begin-var m))
  ((:begin-test-var ct-originals) m))

(defmethod ct/report :end-test-var [m]
  (when (ct-enabled?)
    (ct-on-end-var m))
  ((:end-test-var ct-originals) m))
...
(defmethod ct/report :error [m]
  ((:error ct-originals) m)
  (when (ct-enabled?)
    (ct-on-error m)))

(defmethod ct/report :summary [m]
  ((:summary ct-originals) m)
  (when (and (ct-enabled?) @ct-run-state)
    (ct-on-summary m)))
