  ...
  {"open"
   ...
   (str/join \newline
     ["search - Search Google from the command line"
      ""
      "Output: Markdown table by default. Use --json for machine-readable output."
      "Retries: Automatically retries up to 3 times with backoff on empty results."
      ""
      "Usage:"
...
(defn- client-timeout-for
  "Transport budget for one command: never below `default-client-timeout-ms`,
   never below the daemon's own budget for that action plus slack.

   A flat 30s here against the daemon's 900s budget for `sci_eval`/`ios*` meant
   the CLI reported `Daemon response timed out` while the daemon was still
   working, then wrote its answer into a closed socket. Deriving the value from
   `daemon/client-timeout-ms` keeps the two ends of the transport in agreement."
  ^long [command-map]
  (max (long default-client-timeout-ms)
    (daemon/client-timeout-ms (or (get command-map "action") (get command-map :action)))))
...
(defn send-command!
  "Sends a JSON command to the daemon and returns the parsed response.
   ...
   - positive long: wait up to that many milliseconds
   - nil: use `client-timeout-for`, which sizes the wait from the daemon's own
     per-action budget. It is deliberately NOT \"block forever\": a daemon that
     dies mid-command used to hang the client with no output at all."
  ([^String session command-map]
   (send-command! session command-map (client-timeout-for command-map)))
  ([^String session command-map timeout-ms]
   (let [timeout-ms  (or timeout-ms (client-timeout-for command-map))
         command-map (cond-> command-map
     ...
     (try
       ...
       (let [reader (BufferedReader.
         ...
         (let [f      (future (.readLine reader))
               result (let [r (deref f (long timeout-ms) ::timeout)]
                        (when (= ::timeout r)
                          (future-cancel f)
...
(def ^:private proc-cmdline-os?
  "Whether this OS keeps a process's whole argv in /proc/<pid>/cmdline."
  (str/starts-with? (str/lower-case (System/getProperty "os.name" "")) "linux"))

(defn- process-command-line
  "The full command line of `ph`, or nil when the OS will not tell us.

   `ProcessHandle`'s own answer is not enough on Linux: it is filled from a
   fixed-size read of /proc/<pid>/cmdline, while a daemon started from a JVM
   carries 7288 characters of classpath before its `--session <name>` (measured
   here), so the part that names the session never arrives and the daemon looks
   like a bystander. The kernel keeps the whole argv; macOS and Windows have no
   such limit and answer through the JDK (issue #132).

   Params:
   `ph` - ProcessHandle to describe.

   Returns:
   String command line, or nil."
  ^String [^java.lang.ProcessHandle ph]
  (or (when proc-cmdline-os?
        (try
          (let [p (Path/of (str "/proc/" (.pid ph) "/cmdline") (into-array String []))]
            (when (Files/isReadable p)
              (-> (String. ^bytes (Files/readAllBytes p) java.nio.charset.StandardCharsets/UTF_8)
                (str/replace \u0000 \space)
                str/trim
                not-empty)))
          (catch Exception _ nil)))
    (let [cl (.commandLine (.info ph))]
      (when (.isPresent cl) (str (.get cl))))))
...
(defn orphan-daemon-processes
  "Every running spel daemon process the OS knows about, as `{:pid :session}`.
   ...
   it still holds a browser, a port and hundreds of MB. The process table is
   the only place left to look, read through `process-command-line`: the JDK's
   own view of a command line is cut short on Linux (issue #132).
      ...
      (let [self    (.pid (java.lang.ProcessHandle/current))
            entries (->> (iterator-seq (.iterator (java.lang.ProcessHandle/allProcesses)))
                      (keep (fn [^java.lang.ProcessHandle h]
                              (when-not (= self (.pid h))
                                (daemon-process-entry (.pid h) (process-command-line h)))))
                      (into []))]
  ...
  (try
    (let [handle (java.lang.ProcessHandle/of (Long/parseLong (str/trim (str pid))))]
      (when (.isPresent handle)
        (let [^java.lang.ProcessHandle ph (.get handle)]
          (when (.isAlive ph)
            (daemon-process-entry (.pid ph) (process-command-line ph))))))
    (catch Exception _ nil)))
      ...
      (if success
        (cond
          ...
          (let [entries  (get-in data [:annotated :entries] [])
                ...
                max-mark (reduce max 1 (map #(count (str (:mark %))) entries))]
            ...
            ;; The drawn overlay carries the mark number only — this table is the
            ;; only thing mapping a mark back to a ref, a role and a name.
            (doseq [{:keys [mark ref role name]} entries]
              (println (format (str "  #%-" max-mark "s  @%-" max-ref "s  %-" max-role "s  %s")
                         (str mark)
                         (str ref)
                         (str role)
...
(defn- print-result
  ...
  (let [json-mode? (:json flags)
        ...
        trimmed    (str/replace rendered #"\r?\n\z" "")
