...
(defn set-default-timeout!
  "Sets the default Playwright action timeout (ms) for new pages.
   Called from eval-sci mode when --timeout flag is provided."
  [ms]
  (reset! !default-timeout ms))

;; When true, anomaly results throw instead of being returned.
;; Enables short-circuit error propagation in eval-sci mode.
...
(defn set-throw-on-error!
  "When true, Playwright operations throw on error instead of returning anomaly maps.
   Called from eval-sci mode so errors short-circuit (do ...) forms."
  [v]
...
(defn sci-cdp-connect
  ...
  ([]
   ;; Auto-discover via platform/discover-cdp — shared probe logic
   ;; used by both SCI and daemon. WSL-aware, probes loopback first.
   (if-let [found (platform/discover-cdp)]
     (do
       (binding [*out* *err*]
         (println (str "spel: cdp-connect: " (:browser found)
                    " at " (:host found) ":" (:port found))))
       (sci-cdp-reconnect (str "http://" (:host found) ":" (:port found))))
...
(defn ^:private load-help-registry
  "Loads help data from EDN resource (generated by gen-docs).
   Uses a function instead of delay to avoid build-time realization
   by GraalVM native-image InitClojureClasses."
  []
  (if-let [r (io/resource "com/blockether/spel/help-registry.edn")]
    (edn/read-string (slurp r))
    []))

(def ^:private help-ns-descriptions
  "Human-readable descriptions for each SCI namespace."
  {"spel"     "Simplified API with implicit page"
   "snapshot" "Accessibility snapshot capture"
...
(defn- format-help-table
  ...
  (let [fn-width  30
        arg-width 40
        header    (str (pad-right "Function" fn-width) "  "
                    (pad-right "Arglists" arg-width) "  "
                    "Description")
        sep       (str (apply str (repeat fn-width \-)) "  "
                    (apply str (repeat arg-width \-)) "  "
                    (apply str (repeat 40 \-)))]
    (str header "\n" sep "\n"
      (str/join "\n"
        (for [{:keys [ns name arglists doc]} entries]
          (str (pad-right (str ns "/" name) fn-width) "  "
            (pad-right (truncate arglists arg-width) arg-width) "  "
            (truncate doc 60)))))))
...
(defn create-sci-ctx
  ...
  (let [pw-ns       (sci/create-ns 'spel nil)
        ...
        ;; This lets codegen --format=script output run in eval-sci mode.
        ;; =================================================================
        ...
        ;; generated by `spel codegen --format=script` works in eval-sci.
        ;; =================================================================
...
(defn eval-string
  "Evaluates a Clojure string in the SCI context.
   ...
   `println`, `prn`, etc. work correctly in eval-sci mode.
