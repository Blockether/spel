...
(defn- print-help []
  ...
  (println "")
  (println "Network:")
  (println "  network                          List captured entries (failed requests included)")
  (println "  network get @nN                  Full entry: headers, bodies, duration")
  (println "  network route <url>              Intercept requests (every tab of this session)")
  (println "  network route <url> --abort      Block requests")
  (println "  network route <url> --body <json> Mock response")
  (println "  network unroute [url]            Remove routes from every tab")
  (println "  network requests [flags]         View this tab's requests")
  ...
  (println "    --all                            Every tab of this session, not just this one")
  (println "  network clear                    Clear tracked requests (--all for every tab)")
  (println "")
  (println "Tabs & Windows:")
  (println "  tab                       List tabs (position, id, title, URL)")
  (println "  tab new [url]             New tab")
  (println "  tab <n>                   Switch by position (0-based; shifts when a tab closes)")
  (println "  tab t<n>                  Switch by stable id (the id every entry carries)")
  ...
  (println "  console / console clear   View/clear this tab's console (auto-captured; --all: every tab)")
  (println "  errors / errors clear     View/clear this tab's errors (auto-captured; --all: every tab)")
  ...
  (println "")
  (println "iOS Simulator applications (Appium/XCUITest, macOS only):")
  (println "  click <ref|selector|x y> Native XCTest click (requires --provider ios)")
  (println "  scroll <dir> [amount]    Native touch scroll (requires --provider ios)")
  (println "  eval-sci <code>          Scoped WebView/app/device operations")
  (println "  --provider ios            Drive an app in an iOS Simulator")
  (println "  --bundle-id <id>          Bind an installed application")
  (println "  --app <path>              Install/bind a simulator-built .app")
  (println "  --udid <udid>             Select simulator by UDID")
  (println "  --platform-version <v>    Narrow simulator selection (e.g. 18.2)")
  (println "  --appium-url <url>        Use an external Appium server")
  (println "  --shutdown-simulator      Shut down spel-booted simulator on close")
  (println "")
  (println "Sessions:")
  ...
  (println "  --timeout <ms>            Playwright action timeout in ms (default: 10000)")
...
(defn- env-session
  "Session name from SPEL_SESSION, or nil when unset/blank.

   `spel --help` documents SPEL_SESSION as the default session name, but the
   native global parser never read it, so every eval-sci talked to the
   `default` daemon and quietly polluted the user's own session."
  []
...
(defn- parse-global-flags
  ...
  (loop [remaining (normalize-args args)
         cmd-args  []
         opts      {:timeout-ms nil :debug? false :json? false :autoclose? false :interactive? false :session (env-session) :load-state nil :auto-connect (some? (System/getenv "SPEL_AUTO_CONNECT")) :auto-launch (some? (System/getenv "SPEL_AUTO_LAUNCH")) :browser (System/getenv "SPEL_BROWSER") :channel (System/getenv "SPEL_CHANNEL") :profile (System/getenv "SPEL_PROFILE") :cdp (System/getenv "SPEL_CDP")}]
...
(defn- run-eval!
  ...
  (let [session   (or (:session global) "default")
        ...
        ;; Each Playwright action has its own timeout (--timeout flag, default 10s).
