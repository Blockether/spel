...
(defn- detect-used-namespaces
  "Scans body code string and returns a set of namespace aliases that are actually used.
   Always includes :core (needed for with-testing-page). Detects :assert, :locator, :page, :role."
  [body]
  (let [base #{:core}]
    (cond-> base
      (str/includes? body "assert/") (conj :assert)
      (str/includes? body "locator/") (conj :locator)
      (str/includes? body "page/") (conj :page)
      (str/includes? body "role/") (conj :role))))

(defn- generate-test-header
  "Generates a Lazytest test file header. Only emits requires for namespaces in used-ns."
  [opts used-ns]
  (let [browser-name (or (:browserName opts) "chromium")
        headless? (not (false? (get-in opts [:launchOptions :headless])))
        needs-opts? (or (not= browser-name "chromium") (not headless?))
        opts-map (cond-> {}
                   (not= browser-name "chromium") (assoc :browser-type (keyword browser-name))
                   (not headless?) (assoc :headless false))
        requires (cond-> []
                   (used-ns :assert) (conj "   [com.blockether.spel.assertions :as assert]")
                   true              (conj "   [com.blockether.spel.core :as core]")
                   (used-ns :locator) (conj "   [com.blockether.spel.locator :as locator]")
                   (used-ns :page)    (conj "   [com.blockether.spel.page :as page]")
                   (used-ns :role)    (conj "   [com.blockether.spel.roles :as role]")
                   true              (conj "   [com.blockether.spel.allure :refer [defdescribe it expect]]"))]
    (str
      ";; =============================================================================\n"
      ...
      "  (it \"recorded test\"\n"
      (if needs-opts?
        (str "    (core/with-testing-page " (pr-str opts-map) " [pg]\n")
...
(defn- generate-script-header
  "Generates a standalone script header. Only emits requires for namespaces in used-ns."
  [opts used-ns]
  (let [browser-name (or (:browserName opts) "chromium")
        headless? (not (false? (get-in opts [:launchOptions :headless])))
        needs-opts? (or (not= browser-name "chromium") (not headless?))
        opts-map (cond-> {}
                   ...
                   (not headless?) (assoc :headless false))
        requires (cond-> []
                   (used-ns :assert)  (conj "(require '[com.blockether.spel.assertions :as assert])")
                   true               (conj "(require '[com.blockether.spel.core :as core])")
                   (used-ns :locator) (conj "(require '[com.blockether.spel.locator :as locator])")
                   (used-ns :page)    (conj "(require '[com.blockether.spel.page :as page])")
                   (used-ns :role)    (conj "(require '[com.blockether.spel.roles :as role])"))]
    (str
      ";; Auto-generated by spel codegen\n"
      "\n"
      (str/join "\n" requires)
...
(defn jsonl-str->clojure
  ...
  ([jsonl-str opts]
   (let [lines (remove str/blank? (str/split-lines jsonl-str))
         ...
         used-ns (detect-used-namespaces body)]
     (case fmt
       :test   (str (generate-test-header header-data used-ns) body (generate-test-footer))
       :script (str (generate-script-header header-data used-ns) body (generate-script-footer))
       :body   body))))
...
(defn -main
  ...
  (let [args (vec args)]
    (when (or (empty? args) (some #{"--help" "-h"} args))
      ...
      (println "  spel eval-sci my_script.clj")
      (println "")
      (println "  # 5. Run with saved auth state (cookies/localStorage)")
      (println "  spel eval-sci my_script.clj --load-state auth-state.json")
      (System/exit 0))
    (let [;; Support both --output=FILE, --output FILE, and -o FILE
          ...
          _ (when-let [url (first (filter #(re-matches #"https?://.*" %) file-args))]
              (binding [*out* *err*]
                ...
                (println "")
                (println "To record a browser session:")
                (println (str "  spel codegen record -o recording.jsonl " url))
                (println "")
