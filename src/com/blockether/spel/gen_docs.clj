...
;; =============================================================================
;; Full API Reference Generation (domain-grouped)
;; =============================================================================

(def ^:private full-api-output-path
  "FULL_API.md \u2014 auto-generated domain-grouped reference from help-registry."
  "resources/com/blockether/spel/templates/skills/spel/refs/FULL_API.md")

(def ^:private ns->domain
  "Default domain for each SCI namespace (non-spel)."
  {"core"     "Lifecycle & Setup"
   "input"    "Input Devices"
   "assert"   "Assertions"
   "snapshot" "Snapshots & Accessibility"
   "annotate" "Annotations & Reports"
   "net"      "Network & Routing"
   "frame"    "Frames & Iframes"
   "loc"      "Locator Operations"})

(def ^:private spel-nav-fns
  #{"navigate" "go-back" "go-forward" "reload" "url" "title"
    "wait-for-load-state" "wait-for-url" "wait-for-selector"})

(def ^:private spel-action-fns
  #{"click" "dblclick" "fill" "type" "press-key" "check" "uncheck"
    "select-option" "hover" "tap" "drag-to" "focus" "blur"
    "set-input-files!" "set-checked!"})

(def ^:private spel-content-fns
  #{"text" "html" "attr" "inner-text" "inner-html"
    "all-text-contents" "all-inner-texts" "evaluate" "eval-handle" "content"})

(def ^:private spel-lifecycle-fns
  #{"page" "set-viewport-size!" "close" "set-default-timeout!"
    "set-default-navigation-timeout!" "set-extra-http-headers!"
    "add-init-script!"})

(defn- spel-fn-domain
  "Classifies a spel/ function into a domain by name."
  [fn-name]
  (cond
    (str/starts-with? fn-name "$") "Locators & Selectors"
    (contains? spel-nav-fns fn-name) "Navigation"
    (contains? spel-action-fns fn-name) "Element Actions"
    (contains? spel-content-fns fn-name) "Content & Evaluation"
    (contains? spel-lifecycle-fns fn-name) "Lifecycle & Setup"
    (str/starts-with? fn-name "snapshot") "Snapshots & Accessibility"
    (str/starts-with? fn-name "wait-for") "Navigation"
    (re-find #"screenshot|pdf|video" fn-name) "Annotations & Reports"
    :else "Simplified API"))

(defn- classify-domain
  "Classifies a help-registry entry into a domain."
  [{:keys [ns name]}]
  (if (= ns "spel")
    (spel-fn-domain name)
    (get ns->domain ns "Simplified API")))

(def ^:private domain-display-order
  "Display order for domains in FULL_API.md."
  ["Lifecycle & Setup" "Navigation" "Locators & Selectors"
   "Element Actions" "Content & Evaluation" "Input Devices"
   "Assertions" "Snapshots & Accessibility"
   "Annotations & Reports" "Network & Routing"
   "Frames & Iframes" "Locator Operations" "Simplified API"])

(defn- generate-full-api-md
  "Generates FULL_API.md from help-registry entries, grouped by domain."
  [entries]
  (let [grouped (->> entries
                  (map (fn [e] (assoc e :domain (classify-domain e))))
                  (group-by :domain))
        sections (for [domain domain-display-order
                       :let [domain-entries (get grouped domain)]
                       :when (seq domain-entries)]
                   (str "## " domain "\n\n"
                     "| Function | Args | Description |\n"
                     "|----------|------|-------------|\n"
                     (str/join "\n"
                       (for [{:keys [ns name arglists doc]}
                             (sort-by (juxt :ns :name) domain-entries)]
                         (str "| `" ns "/" name "` | "
                           (escape-md (or arglists "")) " | "
                           (escape-md (or doc "")) " |")))
                     "\n"))
        md (str "# Full API Reference\n\n"
             "Auto-generated by `make gen-docs`. "
             (count entries) " functions grouped by domain.\n\n"
             "**Namespaces:** "
             (str/join ", " (map #(str "`" % "/`") (sort (distinct (map :ns entries)))))
             "\n\n"
             (str/join "\n" sections))]
    (spit (io/file full-api-output-path) md)
    (println "  - Full API reference:" (count entries) "entries in"
      (count (remove nil? (map #(get grouped %) domain-display-order))) "domains")
    (println "  Written to:" full-api-output-path)))

(defn -main
  "CLI entry point."
  [& _args]
  (generate-skill-md)
  (let [entries (generate-help-registry)]
    (generate-full-api-md entries)))
