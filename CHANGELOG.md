...
- Annotate a page the way the agent research does it: a bare NUMBER in the margin instead of a `@ref role name` caption stamped over the element. Marks are placed against the page's own words — text nodes are measured with a Range, so a candidate slot that lands on a title is rejected and the row's whitespace is walked outward from the box before any overlap is accepted. Measured: Hacker News 264 marks, 0 on text (was 135), 0 mark collisions; a 40-field grid 84 marks, 0 on text; Wikipedia 179 marks, 0 on text. The `#N  @ref  role  name` table under the artifact is the legend, and `--dimensions` puts `WxH` back on a mark when the size must be visible in the pixels.
